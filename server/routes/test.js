...
router.post('/llama-vision', upload.single('image'), async (req, res) => {
  try {
    ...
    console.log('\n🧪 TEST: Análisis de Imagen (Local AI - Gratis Sin Límites)');
    console.log('Archivo:', req.file.filename);

    // PRIORIDAD 1: Análisis Local (siempre disponible, sin costo)
    console.log('✅ Usando análisis LOCAL (OpenCV + JIMP) - 0 costo');
    const result = await localAiService.analyzeImage(req.file.path);

    // OPCIONAL: Si HF está configurado y el usuario lo solicita explícitamente
    if (req.query.useHF === 'true' && aiService.isConfigured()) {
      console.log('ℹ️  Usuario solicitó análisis adicional con Hugging Face');
