...
const freeFlightAPIs = require('./freeFlightAPIs');

/**
 * Servicio de Validación Externa
 * Integra APIs externas 100% GRATUITAS para verificar si un avistamiento
 * coincide con vuelos comerciales, satélites u otros objetos conocidos
 * 
 * ACTUALIZADO: Ahora usa múltiples APIs gratuitas (OpenSky, ADS-B Exchange)
 */
class ExternalValidationService {
  
  constructor() {
    // APIs disponibles (TODAS GRATIS)
    this.apis = {
      opensky: 'https://opensky-network.org/api',          // Tráfico aéreo (GRATIS)
      adsbexchange: 'https://globe.adsbexchange.com',      // Tráfico aéreo (GRATIS)
      n2yo: process.env.N2YO_API_KEY,                      // Satélites (opcional)
      celestrak: 'https://celestrak.org/NORAD/elements',   // TLE satélites (GRATIS)
      aviationstack: process.env.AVIATIONSTACK_KEY         // Vuelos (100 req/mes gratis)
    };
  ...
  async checkAircraft(coordinates, timestamp, altitude = null) {
    try {
      ...
      console.log('✈️ Consultando múltiples APIs GRATUITAS de vuelos...');

      // Usar nuevo servicio con múltiples APIs gratis
      const flightData = await freeFlightAPIs.getAllFlights(
        coordinates,
        timestamp,
        { aviationStackKey: this.apis.aviationstack }
      );

      const matches = [];
      const { lat, lng } = coordinates;

      // Procesar vuelos de todas las fuentes
      for (const flight of flightData.flights) {
        if (flight.latitude && flight.longitude) {
          ...
          // Considerar coincidencia si está a menos de 100km
          if (distance <= 100) {
            matches.push({
              source: flight.source || 'Multiple Free APIs',
              callsign: flight.callsign,
              icao24: flight.icao24,
              origin: flight.country || flight.airline,
              ...
              timestamp: flight.lastSeen || timestamp,
              confidence: this.calculateAircraftConfidence(distance, altitude, flight.altitude)
            });
          }
        }
      }
